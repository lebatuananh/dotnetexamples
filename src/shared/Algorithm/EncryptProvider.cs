...
using Shared.Algorithm.Internal;

namespace Shared.Algorithm
{
    public class EncryptProvider
    {
        ...
        */
        ...
        public static byte[] AESDecrypt(byte[] data, string key, string vector)
        {
            ...
            using (Aes Aes = Aes.Create())
            {
                try
                {
                    ...
                    {
                        using (CryptoStream decryptor = new CryptoStream(memory, Aes.CreateDecryptor(bKey, bVector),
        ...
        /// <param name="vector">IV,requires 8 bits</param>
        /// <returns>Encrypted byte array</returns>
        private static byte[] DESEncrypt(byte[] data, string key, CipherMode cipherMode, string vector = "",
            PaddingMode paddingMode = PaddingMode.PKCS7)
        {
            ...
            using (MemoryStream Memory = new MemoryStream())
            {
                using (TripleDES des = TripleDES.Create())
                {
                    ...
                    using (CryptoStream cryptoStream =
                           new CryptoStream(Memory, des.CreateEncryptor(), CryptoStreamMode.Write))
                    {
                        try
                        {
                            cryptoStream.Write(plainBytes, 0, plainBytes.Length);
        ...
        /// <param name="paddingMode"><see cref="PaddingMode"/> default is PKCS7</param>
        /// <returns>Decrypted byte array</returns>
        private static byte[] DESDecrypt(byte[] data, string key, CipherMode cipherMode, string vector = "",
            PaddingMode paddingMode = PaddingMode.PKCS7)
        {
            Check.Argument.IsNotEmpty(data, nameof(data));
            Check.Argument.IsNotEmpty(key, nameof(key));
            ...
            using (MemoryStream Memory = new MemoryStream(encryptedBytes))
            {
                using (TripleDES des = TripleDES.Create())
                {
                    ...
                    using (CryptoStream cryptoStream =
                           new CryptoStream(Memory, des.CreateDecryptor(), CryptoStreamMode.Read))
                    {
                        try
                        {
        ...
        /// <returns></returns>
        public static string RSASign(string content, string privateKey, HashAlgorithmName hashAlgorithmName,
            RSASignaturePadding rSASignaturePadding, Encoding encoding)
        {
        ...
        {
            return RSAVerify(content, signStr, publickKey, HashAlgorithmName.SHA256, RSASignaturePadding.Pkcs1,
                Encoding.UTF8);
        }
        ...
        /// <returns></returns>
        public static bool RSAVerify(string content, string signStr, string publickKey,
            HashAlgorithmName hashAlgorithmName, RSASignaturePadding rSASignaturePadding, Encoding encoding)
        {
            Check.Argument.IsNotEmpty(content, nameof(content));
            ...
            bool isPemKey = false)
        {
            ...
            using (rsa)
            {
                ...
                {
                    throw new OutofMaxlengthException($"'{srcString}' is out of max encrypt length {maxLength}",
                        maxLength, rsa.KeySize, padding);
                }
        ...
        /// <param name="isPemKey">set key is pem format,default is false</param>
        /// <returns>encrypted byte[]</returns>
        public static byte[] RSAEncrypt(string publicKey, byte[] data, RSAEncryptionPadding padding,
            bool isPemKey = false)
        {
            Check.Argument.IsNotEmpty(publicKey, nameof(publicKey));
            ...
            using (rsa)
            {
                ...
                if (rawBytes.Length > maxLength)
                {
                    throw new OutofMaxlengthException($"data is out of max encrypt length {maxLength}", maxLength,
                        rsa.KeySize, padding);
                }
        ...
        /// <param name="isPemKey">set key is pem format,default is false</param>
        /// <returns>encrypted string</returns>
        public static byte[] RSADecrypt(string privateKey, byte[] data, RSAEncryptionPadding padding,
            bool isPemKey = false)
        {
            Check.Argument.IsNotEmpty(privateKey, nameof(privateKey));
            Check.Argument.IsNotNull(data, nameof(data));

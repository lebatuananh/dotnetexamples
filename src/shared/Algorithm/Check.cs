...
public struct Check
{
    ...
    public class Argument
    {
        ...
        public static void IsNotOutOfLength(string argument, int length, string argumentName)
        {
            ...
            {
                throw new ArgumentException(string.Format("\"{0}\" no more than {1} characters.", argumentName, length),
                    argumentName);
            }
        }
        ...
        public static void IsNotOutOfRange(int argument, int min, int max, string argumentName)
        {
            ...
            {
                throw new ArgumentOutOfRangeException(argumentName,
                    string.Format("{0} must be in the range \"{1}\"-\"{2}\".", argumentName, min, max));
            }
        }
        ...
        public static void IsEqualLength(int sourceLength, int limitLength, string argumentName)
        {
            if (limitLength != sourceLength)
            {
                throw new ArgumentException(argumentName,
                    string.Format("The length of arugment {0} must be equal to {1}.", argumentName, limitLength));
            }
        }
        ...
        public static void IsNotExistsDirectory(string argument, string argumentName)
        {
            ...
            {
                throw new ArgumentException(string.Format("\"{0}\" directory does not exist", argumentName),
                    argumentName);
            }
        }
    }

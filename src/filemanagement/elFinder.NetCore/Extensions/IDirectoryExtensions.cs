...
namespace elFinder.NetCore.Extensions
{
    public static class IDirectoryExtensions
    {
        ...
        private static byte GetFlag(this IDirectory directory, RootVolume volume,
            Func<AccessControlAttributeSet, bool> fieldSelector)
        {
            if (volume.AccessControlAttributes != null)
            {
                ...
                if (attributeSet != null) return fieldSelector(attributeSet) ? (byte)1 : (byte)0;

                var parentDirectory = directory.Parent;
                while (parentDirectory != null && parentDirectory.FullName != volume.RootDirectory)
                {
                    attributeSet =
                        volume.AccessControlAttributes.FirstOrDefault(x => x.FullName == parentDirectory.FullName);
                    if (attributeSet != null) return fieldSelector(attributeSet) ? (byte)1 : (byte)0;

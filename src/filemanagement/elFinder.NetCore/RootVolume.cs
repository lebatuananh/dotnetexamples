...
namespace elFinder.NetCore
{
    ...
    public class RootVolume
    {
        ...
        ///     Character used to separate directory levels in a path string. Default is
            ...
            char directorySeparatorChar = default)
        {
            ...
            if (!string.IsNullOrEmpty(thumbnailsUrl)) ThumbnailUrl = thumbnailsUrl;
        ...
        public AccessControlAttributeSet DefaultAccessControlAttributes
        {
            ...
            set => defaultAccessControlAttributes =
                value ?? throw new ArgumentNullException(nameof(DefaultAccessControlAttributes));
        }
        ...
        {
            get => MaxUploadSize.HasValue ? (double?)(MaxUploadSize.Value / 1024.0) : null;
        ...
        {
            get => MaxUploadSizeInKb.HasValue ? (double?)(MaxUploadSizeInKb.Value / 1024.0) : null;
        ...
        /// <summary>
        ///     Replace files with the same name on upload or give them new names. true - replace old files, false give new names
        ///     like original_name-number.ext
        /// </summary>
        public bool UploadOverwrite { get; }

        /// <summary>
        ///     URL that points to path directory (also called 'root URL'). If not set client will not see full path to files
        ///     (replacement
        ...
        /// <summary>
        ///     Order to proccess uploadAllow and uploadDeny options. Logic is the same as Apache web server options Allow, Deny,
        ///     Order
        /// </summary>
        ...
        public async Task<string> GenerateThumbHashAsync(IFile originalImage)
        {
            ...
            {
                var md5 = await originalImage.GetFileMd5Async();
                var thumbName =
                    $"{Path.GetFileNameWithoutExtension(originalImage.Name)}_{md5}{originalImage.Extension}";
                var relativePath = originalImage.DirectoryName.Substring(RootDirectory.Length);
                return VolumeId + HttpEncoder.EncodePath($"{relativePath}{DirectorySeparatorChar}{thumbName}");
            }
            else
            {
                var thumbPath = await GenerateThumbPathAsync(originalImage);
                var relativePath = thumbPath.Substring(ThumbnailDirectory.Length);
        ...
        {
            if (ThumbnailDirectory == null || !CanCreateThumbnail(originalImage)) return null;
            var relativePath = originalImage.FullName.Substring(RootDirectory.Length);
            var thumbDir = GetDirectoryName($"{ThumbnailDirectory}{relativePath}");
            var md5 = await originalImage.GetFileMd5Async();
            var thumbName = $"{Path.GetFileNameWithoutExtension(originalImage.Name)}_{md5}{originalImage.Extension}";
            return $"{thumbDir}{DirectorySeparatorChar}{thumbName}";
        }

        public string GenerateThumbPath(IDirectory originalDirectory)
        {
            if (ThumbnailDirectory == null) return null;
            var relativePath = originalDirectory.FullName.Substring(RootDirectory.Length);
        ...
        private string GetDirectoryName(string file)
        {
            ...
            while (--startIndex >= 0)
            {
                ...
                if (ch == DirectorySeparatorChar) return file.Substring(0, startIndex);
            }
        ...
        {
            var thumbPath = await GetExistingThumbPathAsync(originalImage);
        ...
        internal string GetExistingThumbPath(IDirectory originalDirectory)
        {
            ...
            var relativePath = originalDirectory.FullName.Substring(RootDirectory.Length);
            var thumbDir = ThumbnailDirectory + relativePath;
            return thumbDir;
        }
    }
}
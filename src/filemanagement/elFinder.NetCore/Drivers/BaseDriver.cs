...
namespace elFinder.NetCore.Drivers
{
    ...
    public abstract class BaseDriver
    {
        ...
        protected virtual async Task AddDirectoryToArchiveAsync(ZipArchive zipFile, IDirectory directoryInfo,
            string root)
        {
            var entryName = $"{root}{directoryInfo.Name}/";

            zipFile.CreateEntry(entryName);
            var dirs = await directoryInfo.GetDirectoriesAsync();

            foreach (var dir in dirs) await AddDirectoryToArchiveAsync(zipFile, dir, entryName);

            var files = await directoryInfo.GetFilesAsync(null);
            foreach (var file in files) zipFile.CreateEntryFromFile(file.FullName, entryName + file.Name);
        }

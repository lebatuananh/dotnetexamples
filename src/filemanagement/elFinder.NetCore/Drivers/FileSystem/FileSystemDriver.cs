...
namespace elFinder.NetCore.Drivers.FileSystem
{
    ...
    public class FileSystemDriver : BaseDriver, IDriver
    {
        ...
        private static string CreateNameForCopy(FileInfo file, string suffix)
        {
            ...
            if (string.IsNullOrEmpty(suffix)) suffix = "copy";

            var newName = $"{parentPath}{Path.DirectorySeparatorChar}{name} {suffix}{extension}";
        ...
        private async Task RemoveThumbsAsync(FullPath path)
        {
            if (path.IsDirectory)
            {
                var thumbPath = path.RootVolume.GetExistingThumbPath(path.Directory);
                if (!string.IsNullOrEmpty(thumbPath) && Directory.Exists(thumbPath)) Directory.Delete(thumbPath, true);
            }
            else
            {
                var thumbPath = await path.RootVolume.GetExistingThumbPathAsync(path.File);
                if (!string.IsNullOrEmpty(thumbPath) && File.Exists(thumbPath)) File.Delete(thumbPath);
            }
        }

        #region IDriver Members

        public async Task<JsonResult> ArchiveAsync(FullPath parentPath, IEnumerable<FullPath> paths, string filename,
            string mimeType)
        {
        ...
        public async Task<JsonResult> DuplicateAsync(IEnumerable<FullPath> paths)
        {
            ...
            foreach (var path in paths)
                if (path.IsDirectory)
                {
                    ...
                    var newName = $"{parentPath}{Path.DirectorySeparatorChar}{name} copy";
                    ...
                    else
                    {
                        ...
                        if (!foundNewName) return Error.NewNameSelectionException($"{name} copy");
                    }
                ...
                else
                {
                    ...
                    var newName = $"{parentPath}{Path.DirectorySeparatorChar}{name} copy{ext}";
                    ...
                    else
                    {
                        ...
                        if (!foundNewName) return Error.NewNameSelectionException($"{name} copy{ext}");
                    }
                ...
                foreach (var entry in archive.Entries)
                    try
                    {
                        ...
                        else
                        {
                            ...
                            if (!newFolder)
                                response.Added.Add(await BaseModel.CreateAsync(new FileSystemFile(file),
                                    fullPath.RootVolume));
                        }
                    }
                    catch //(Exception ex)
        ...
        public async Task<IActionResult> FileAsync(FullPath path, bool download)
        {
            ...
            if (!await path.File.ExistsAsync) return new NotFoundResult();
        ...
        public async Task<JsonResult> InitAsync(FullPath path, IEnumerable<string> mimeTypes)
        {
            ...
            var response = new InitResponseModel(await BaseModel.CreateAsync(path.Directory, path.RootVolume),
                new Options(path));
            ...
            IEnumerable<string> mimeTypes)
        {
            ...
            if (intersect.Any()) response.List.RemoveAll(x => !intersect.Contains(x));
        ...
        public async Task<FullPath> ParsePathAsync(string target)
        {
            ...
            var underscoreIndex = target.IndexOf('_');
            var pathHash = target.Substring(underscoreIndex + 1);
            var volumePrefix = target.Substring(0, underscoreIndex + 1);

            var root = Roots.First(r => r.VolumeId == volumePrefix);
            var rootDirectory = new DirectoryInfo(root.RootDirectory);
            var path = HttpEncoder.DecodePath(pathHash);
            var dirUrl = path != rootDirectory.Name ? path : string.Empty;
        ...
        public async Task<JsonResult> PasteAsync(FullPath dest, IEnumerable<FullPath> paths, bool isCut,
            IEnumerable<string> renames, string suffix)
        {
            ...
            foreach (var src in paths)
                if (src.IsDirectory)
                {
                    ...
                    if (await newDir.ExistsAsync) Directory.Delete(newDir.FullName, true);
                ...
                else
                {
                    ...
                    if (await newFile.ExistsAsync) await newFile.DeleteAsync();
        ...
        public async Task<JsonResult> SearchAsync(FullPath path, string query, IEnumerable<string> mimeTypes)
        {
            ...
            if (!query.Any(Path.GetInvalidFileNameChars().Contains))
            {
                foreach (var item in await path.Directory.GetFilesAsync(mimeTypes, string.Concat("*", query, "*")))
                    if (!item.Attributes.HasFlag(FileAttributes.Hidden) &&
                        !item.Directory.Attributes.HasFlag(FileAttributes.Hidden))
                        response.Files.Add(await BaseModel.CreateAsync(item, path.RootVolume));
        ...
        public async Task<JsonResult> UploadAsync(FullPath path, IEnumerable<IFormFile> files, bool? overwrite,
            IEnumerable<FullPath> uploadPaths, IEnumerable<string> renames, string suffix)
        {
            var response = new AddResponseModel();
            ...
            foreach (var rename in renames)
            {
                ...
                var destination = Path.Combine(path.Directory.FullName,
                    $"{Path.GetFileNameWithoutExtension(rename)}{suffix}{Path.GetExtension(rename)}");
            ...
            foreach (var uploadPath in uploadPaths)
            {
                ...
                {
                    response.Added.Add(await BaseModel.CreateAsync(new FileSystemDirectory(directory.FullName),
                        path.RootVolume));
            ...
            foreach (var file in files)
            {
                var destination = uploadPaths.Count() > i
                    ? uploadPaths.ElementAt(i).Directory.FullName
                ...
                if (fileInfo.Exists)
                {
                    if (overwrite ?? path.RootVolume.UploadOverwrite)
                    {
                        ...
                        response.Added.Add(await BaseModel.CreateAsync(new FileSystemFile(fileInfo.FullName),
                            path.RootVolume));
                    }
                    else
                    {
                        var newName = CreateNameForCopy(fileInfo, suffix);
                        using (var fileStream = new FileStream(Path.Combine(fileInfo.DirectoryName, newName),
                                   FileMode.Create))
                        {
                            await file.CopyToAsync(fileStream);
                        }
        ...
        public async Task<FileStreamResult> ZipDownloadAsync(FullPath cwdPath, string archivedFileKey,
            string downloadFileName, string mimeType)
        {

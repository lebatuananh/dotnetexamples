...
namespace elFinder.NetCore.Drivers.FileSystem
{
    public class FileSystemDirectory : IDirectory
    {
        ...
        {
            var dirs = directoryInfo.GetDirectories(pattern, SearchOption.AllDirectories)
                .Select(d => new FileSystemDirectory(d) as IDirectory);
            return Task.FromResult(dirs);
        }
        ...
        public Task<IEnumerable<IFile>> GetFilesAsync(IEnumerable<string> mimeTypes, string pattern)
        {
            var files = directoryInfo.GetFiles(pattern, SearchOption.AllDirectories)
                .Select(f => new FileSystemFile(f) as IFile);

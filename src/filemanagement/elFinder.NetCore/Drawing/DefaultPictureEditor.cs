...
namespace elFinder.NetCore.Drawing
{
    ...
    public class DefaultPictureEditor : IPictureEditor
    {
        ...
        /// <returns>
        ///     A new <see cref="System.Drawing.Bitmap" /> that is just large enough to contain the rotated image without
        ///     cutting any corners off.
        /// </returns>
        ...
        private ImageWithMimeType Rotate(Image image, float degrees)
        {
            ...
            while (lockedTheta < 0.0) lockedTheta += 2 * Math.PI;
        ...
        {
            var ext = originalImageExtension.ToLower();
            if (ext == ".tiff") return ".png";

            if (ext == ".png" || ext == ".jpg" || ext == ".jpeg" || ext == ".gif")
                return ext;
            throw new ArgumentException(typeof(DefaultPictureEditor).FullName + " does not support thumbnails for '" +
                                        originalImageExtension + "' files.");
        }
        ...
        public ImageWithMimeType GenerateThumbnail(Stream input, int size, bool keepAspectRatio)
        {
            using (var inputImage = Image.FromStream(input))
            {
                ...
                if (keepAspectRatio)
                {
                    ...
                    var percentWidth = width != 0 ? size / width : 0;
                    var percentHeight = height != 0 ? size / height : 0;
        ...
        public ImageWithMimeType Resize(Stream input, int width, int height)
        {
            using (var image = Image.FromStream(input))
            {
                return ScaleOrCrop(image, new Rectangle(0, 0, image.Width, image.Height),

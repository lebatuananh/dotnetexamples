...
namespace elFinder.NetCore
{
    ...
    public class Connector
    {
        ...
        {
            var parameters = request.Query.Any(x => x.Key != "access_token")
            ...
            if (string.IsNullOrEmpty(cmd)) return Error.UnknownCommand();
            ...
            switch (cmd)
            {
                ...
                {
                    var path = await driver.ParsePathAsync(parameters.GetValueOrDefault("target"));
                    return new JsonResult(new { Url = path.RootVolume.Url + path.RelativePath });
                }
                case "ls":
                {
                    var path = await driver.ParsePathAsync(parameters.GetValueOrDefault("target"));
                ...
                case "size":
                {
                    var paths = new StringValues(parameters.Where(p => p.Key.StartsWith("target"))
                        .Select(p => (string)p.Value).ToArray());
                ...
                case "upload":
                {
                    ...
                    var overwrite = parameters.GetValueOrDefault("overwrite") != "0";
                ...
                case "zipdl":
                {
                    ...
                    var archiveFileKey = targets[1];
                    var downloadFileName = targets[2];
                    var mimeType = targets[3];
        ...
        {
            return !string.IsNullOrEmpty(path.RootVolume.ThumbnailUrl) &&
                   pictureEditor.CanProcessFile(path.File.Extension);
        }

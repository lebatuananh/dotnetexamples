...
namespace elFinder.NetCore.Models
{
    public abstract class BaseModel
    {
        ...
        public static async Task<FileModel> CreateAsync(IFile file, RootVolume volume)
        {
            ...
            response.ParentHash = volume.VolumeId +
                                  HttpEncoder.EncodePath(parentPath.Length > 0 ? parentPath : file.Directory.Name);
            return response;
        }
        ...
        public static async Task<DirectoryModel> CreateAsync(IDirectory directory, RootVolume volume)
        {
            ...
            {
                var parentPath = directory.Parent.FullName.Substring(volume.RootDirectory.Length);
                var relativePath = directory.FullName.Substring(volume.RootDirectory.Length)
                ...
                var response = new DirectoryModel
                {
                    ...
                    ParentHash = volume.VolumeId +
                                 HttpEncoder.EncodePath(parentPath.Length > 0 ? parentPath : directory.Parent.Name),
                    VolumeId = volume.VolumeId
                };
                return response;
            }

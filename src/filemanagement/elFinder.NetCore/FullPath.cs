...
namespace elFinder.NetCore
{
    public class FullPath
    {
        ...
        public async Task<ImageWithMimeType> GenerateThumbnailAsync()
        {
            ...
            var fullPath = $"{name}{File.Extension}";
            ...
            if (RootVolume.ThumbnailDirectory != null)
            {
                ...
                if (File.FullName.StartsWith(RootVolume.ThumbnailDirectory))
                    ...
                    thumbFile = File.Open(
                        $"{RootVolume.ThumbnailDirectory}{RootVolume.DirectorySeparatorChar}{RelativePath}");
                ...
                if (!await thumbFile.ExistsAsync)
                {
                    ...
                    {
                        var thumb = RootVolume.PictureEditor.GenerateThumbnail(original, RootVolume.ThumbnailSize,
                            true);
                ...
                string mimeType =
                    MimeHelper.GetMimeType(RootVolume.PictureEditor.ConvertThumbnailExtension(thumbFile.Extension));
        ...
        public FullPath(RootVolume root, IFile file, string hashedTarget) : this(root, file.FullName, hashedTarget,
            false)
        {
            File = file ?? throw new ArgumentNullException("file", "IFile cannot be null");
        }

        public FullPath(RootVolume root, IDirectory dir, string hashedTarget) : this(root, dir.FullName, hashedTarget,
            true)
        {
